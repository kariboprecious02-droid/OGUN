...
- Why `provider_call_state: timed_out` rows do not currently enqueue a polling job (`collection.service.ts:295` gates polling on `next_action !== null`)
...
1. **PR-1** — Lifecycle observability foundation (L1 + L2 bundled). Migration `0006_collection_events.sql` + axios interceptors on both Paystack connectors + structured `recordCollectionEvent` calls at every lifecycle gate. Acceptance criteria are in §7 PR-1.
